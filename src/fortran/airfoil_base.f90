...
module airfoil_base
  ...
  type airfoil_type 
    ...
    double precision, allocatable :: hh_seed_top_y(:)    ! seed y coordinates for independant rebuild(write)  
    double precision, allocatable :: hh_seed_bot_y(:)              
...
contains
  ...
  subroutine airfoil_write_with_shapes (foil, output_dir, highlight)
    ...
    logical                         :: do_highlight
    double precision, allocatable   :: seed_x(:), seed_y(:) 
    integer                         :: np, npt
    ...
    ! write hicks-henne .hicks 
      ...
      ! rebuild x,y of seed airfoil to write it in .hicks file 

      npt = size(foil%top%x)
      np  = size(foil%top%x) + size(foil%bot%x) - 1
      allocate(seed_x(np))
      allocate(seed_y(np))
  
      seed_x(1:npt) = foil%top%x (npt:1:-1)
      seed_y(1:npt) = foil%hh_seed_top_y (npt:1:-1)

      seed_x(npt:)  = foil%bot%x 
      seed_y(npt:)  = foil%hh_seed_bot_y 
  
      call write_hh_file (path_join (output_dir, fileName), foil%top_hh, foil%bot_hh, &
                          foil%hh_seed_name, seed_x, seed_y)
