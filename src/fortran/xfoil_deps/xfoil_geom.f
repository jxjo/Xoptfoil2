...
C  Copyright (C) 2017-2019 Daniel Prosser (this modified version of 
C  XFoil code)
...
C      CALL TCCALC(X,XP,Y,YP,S,N,SILENT_MODE,
C     &            THICKB,XTHICKB,THICKM,XTHICKM,CAMBR,XCAMBR)
C
      RETURN
      END ! PANGEN
...
C     DP mod: added SILENT_MODE option
C     jx mod: no message - 
C      IF(LCONF .AND. (.NOT. SILENT_MODE)) WRITE(*,1200) N
...
C     JX-mod suppress error message in case of invalid geometry
C      WRITE(*,*)
...
C     JX-mod
      YMAX = 0.0E0
      XMAX = XLEN / 2
C     WRITE (*,'(A,F7.4,A,F7.4)') 'YMAX= ',YMAX, 'XMAX= ',XMAX
C
 20   RETURN 
      END ! GETMAX
